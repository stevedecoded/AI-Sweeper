package  
{
	...
	public class BruteGridAI
	{
		...
		public function BruteGridAI(rows:uint = 8, cols:uint = 8, bombs:uint = 10)
		{
		...
		public function get solution():Array { return _proxGrid; }
		...
		public function initAdvancedGrid():Boolean
		{
			return initGrid(10, 10, 12);
		}
		...
		private function findBestMove(probs:Array):Array
		{
			...
			var lowProb:int = 101;
			var bruteClick:Array = [-1, -1];
			var bruteStartCell:Array = [-1, -1];
			...
			for (var row:int = 0; row < _boardRows; row++)
			{
				for (var col:int = 0; col < _boardCols; col++)
				{
					...
					else if (!isFound(bruteStartCell) && probs[row][col] > 0)
					{
						bruteStartCell = [row, col];
					}
			...
			{
				if (isFound(bruteStartCell))
				{
					bruteClick = findBruteClick(probs, bruteStartCell);
					
					if (isFound(bruteClick))
					{
						return [bruteClick[0], bruteClick[1], ACTION_BRUTE];
					}
				}
				
				if (isFound(emptyCell))
				{
		...
		private function findBruteClick(probs:Array, startCell:Array):Array
		{
			var currentCol:int = startCell[1];
			
			for (var row:int = startCell[0]; row < _boardRows; row++)
			{
				for (var col:int = currentCol; col < _boardCols; col++)
				{
					if (probs[row][col] >= 0)
					{
						var tempProbs:Array = copy2DArray(probs);
						
						var defMove:Array;
						var defCell:Array = [ -1, -1];
						
						setFlagged(row, col, tempProbs);
						
						recalcProb(tempProbs);
						
						if (!checkTempProxs(tempProbs))
						{
							return [row, col];
						}
						...
						do
						{
							defMove = findDefiniteMove(tempProbs);
							defCell = [defMove[0], defMove[1]];
							
							if (defMove[2] == ACTION_CLICK)
							{
								setClicked(defCell[0], defCell[1], tempProbs);
							}
							else if (defMove[2] == ACTION_FLAG)
							{
								setFlagged(defCell[0], defCell[1], tempProbs);
							}
		...
		private function findDefiniteMove(probs:Array):Array
		{
			var definiteClick:Array = [-1, -1];
			...
			return [-1, -1, ACTION_NO_DEFINITES];
			// returns when no definite moves are available in grid
		}
		
		private function checkTempProxs(probs:Array):Boolean
		{
			for (var row:int = 0; row < _boardRows; row++)
			{
				for (var col:int = 0; col < _boardCols; col++)
				{
					if (!checkCellProx(row, col, probs))
					{
						return false;
					}
				}
		...
		private function checkCellProx(row:int, col:int, probs:Array):Boolean
		{
			if (_publicProxGrid[row][col] >= 0)
			{
				...
				for each (var cell:Array in surCells)
				{
					if (probs[row + cell[0]][col + cell[1]] == -3)
					{
						flags++;
					}
					else if(probs[row + cell[0]][col + cell[1]] == -2)
					{
						clicks++;
					}
					else if (probs[row + cell[0]][col + cell[1]] >= -1)
					{
						empties++;
					}
				}
				
				if(flags > _publicProxGrid[row][col] || (empties == 0 && flags < _publicProxGrid[row][col]))
				{
					return false;
				}
			}
		...
		public function copy2DArray(arr:Array):Array
		{
			var copy:Array = [];
